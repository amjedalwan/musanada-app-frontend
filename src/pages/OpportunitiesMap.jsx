...
const userIcon = L.divIcon({
    ...
    html: `
        <div class="relative">
            <div class="absolute inset-[-8px] rounded-full bg-emerald-500/30 animate-ping"></div>
...
const getOppIcon = (type, isSelected) => L.divIcon({
    ...
    html: `
        ...
        } shadow-lg border-2 border-slate-300">
...
const OpportunitiesMap = () => {
    ...
    const handleShowDetails = (opp) => {
        MySwal.fire({
            html: (
                <div className="flex flex-col text-right font-['Tajawal']" dir="rtl">
                    <div className="relative w-full h-48 rounded-3xl overflow-hidden mb-5 group shadow-lg">
                        ...
                        <div className="absolute bottom-4 right-4 text-slate-900">
                            <span className="text-[10px] bg-emerald-600 px-3 py-1 rounded-full font-black uppercase tracking-widest shadow-lg">
                                {opp.type === 'voluntary' ? 'تطوعي' : 'تدريبي'}
                            </span>
                        </div>
                    </div>

                    <h2 className="text-xl font-black text-slate-900 mb-2 leading-tight">{opp.title}</h2>
            ...
            width: '400px',
            customClass: { popup: 'rounded-[2.5rem] border border-slate-200 p-2' }
        });
    };

    return (
        <div className="flex h-screen bg-slate-50 text-slate-900 font-['Tajawal'] overflow-hidden selection:bg-emerald-500/30" dir="rtl">
            ...
            <main className="flex-1 flex flex-col relative">
                ...
                <div className="absolute top-6 inset-x-6 z-[1000] flex flex-col lg:flex-row gap-4 pointer-events-none">
                    ...
                    >
                        <div className="bg-[#11111a]/90 backdrop-blur-2xl border border-slate-200 p-2 rounded-[2rem] shadow-2xl flex items-center gap-2">
                            <div className="relative flex-1">
                                <Search className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                                <input 
                                    type="text"
                                    placeholder="ابحث عن الفرص في منطقتك..."
                                    className="w-full bg-white border border-slate-100 pr-12 pl-4 py-3.5 rounded-[1.5rem] outline-none text-sm font-bold focus:ring-2 ring-indigo-500/20 transition-all"
                                    value={searchTerm}
                            ...
                            <div className="hidden sm:flex items-center gap-1 bg-slate-50 p-1 rounded-xl border border-slate-100">
                                {['all', 'voluntary', 'training'].map((t) => (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg text-[10px] font-black transition-all ${filterType === t ? 'bg-emerald-600 text-slate-900' : 'text-slate-400 hover:text-slate-600'}`}
                                    >
                ...
                <div className="flex-1 relative z-10 group">
                                ...
                                >
                                    <Popup className="custom-popup" closeButton={false}>
                                        <div className="text-right p-1 font-['Tajawal']">
                                            <p className="font-black text-emerald-600 text-sm mb-1">{opp.title}</p>
                                            <p className="text-[10px] text-slate-500 mb-2">{opp.distance} كم من موقعك</p>
                                            <button 
                                                onClick={() => handleShowDetails(opp)}
                                                className="w-full bg-emerald-600 py-1.5 rounded-lg text-[10px] font-black text-slate-900"
                                            >
                                                التفاصيل السريعة
                                            </button>
                                        </div>
                    ...
                    <AnimatePresence>
                        {isSidebarOpen && (
                            <motion.aside 
                                ...
                                className="absolute right-6 top-28 bottom-24 w-[350px] bg-white/95 backdrop-blur-3xl border border-slate-200 rounded-[2.5rem] z-[1001] shadow-2xl p-6 flex flex-col"
                            >
                                <div className="flex items-center justify-between mb-6">
                                    <div>
                                        <h3 className="font-black text-lg">الأقرب إليك</h3>
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">مرتبة جغرافياً حسب موقعك الحالي</p>
                                    </div>
                                    <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="hover:bg-red-400/50  p-3 bg-slate-50 rounded-full cursor-pointer">
                                        <X size={18} />
                                    </button>
                                </div>
                                        ...
                                        opportunities.map(opp => (
                                            <div 
                                                ...
                                                className={`p-4 rounded-md border transition-all cursor-pointer group ${
                                                    selectedOpp?.id === opp.id ? 'bg-emerald-600/10 border-emerald-500/50 shadow-lg shadow-indigo-500/10' : 'bg-slate-50 border-slate-100 hover:bg-slate-50'
                                                }`}
                                            >
                                                ...
                                                <h4 className="font-bold text-sm text-slate-900 group-hover:text-slate-900 transition-colors mb-2 line-clamp-1">{opp.title}</h4>
                                ...
                                <div className="mt-6 bg-emerald-600 p-5 rounded-md relative overflow-hidden group">
                                    <MapIcon size={60} className="absolute -left-2 -bottom-2 text-slate-900/10 rotate-12 transition-transform group-hover:scale-125" />
                                    <p className="text-[10px] font-black text-slate-900/60 mb-1">الفرص المكتشفة</p>
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-3xl font-black">{opportunities.length}</span>
                                        <span className="text-xs font-bold text-slate-900/80 uppercase">فرصة نشطة</span>
                                    </div>
                                </div>
                            </motion.aside>
                        )}
                    ...
                    <div className="absolute left-6 bottom-10 z-[1000] flex gap-2">
                        <button 
                            ...
                            className="bg-white border border-slate-200 px-5 py-3.5 rounded-2xl shadow-2xl hover:bg-emerald-600 transition-all text-[11px] font-black uppercase flex items-center gap-3"
                        >
                ...
                <AnimatePresence>
                    {selectedOpp && (
                        <motion.div 
                            ...
                            className="absolute bottom-8 left-1/2 z-[1002] w-[90%] max-w-lg bg-white/98 backdrop-blur-3xl border border-emerald-500/30 p-4 rounded-[2.5rem] shadow-2xl flex items-center gap-4"
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <h4 className="text-sm font-black text-slate-900 truncate mb-0.5">{selectedOpp.title}</h4>
                                <p className="text-[11px] text-slate-400 font-bold truncate">{selectedOpp.user?.organization?.org_name}</p>
                            ...
                            <div className="flex gap-1.5">
                                <button onClick={() => handleShowDetails(selectedOpp)} className="p-3 bg-emerald-600 rounded-xl hover:bg-emerald-500 transition-colors"><Info size={18} /></button>
                                <button onClick={() => setSelectedOpp(null)} className="p-3 bg-slate-50 rounded-xl hover:bg-red-500/20 text-slate-500 hover:text-red-500 transition-colors"><X size={18} /></button>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
...
const ControlBtn = ({ icon: Icon, onClick, active, className = "" }) => (
    <button 
        onClick={onClick}
        className={`p-4 rounded-2xl shadow-2xl transition-all active:scale-90 border border-slate-200 ${
            active ? 'bg-emerald-600 text-slate-900' : 'bg-[#11111a]/90 text-slate-500 hover:text-slate-900'
        } ${className}`}
    >
        <Icon size={22} />
    </button>
