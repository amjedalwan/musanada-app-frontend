...
const ApplicationCard = ({ app, idx, getStatusDetails, navigate }) => {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-[#0f0f1a]/40 border border-white/5 rounded-[2.5rem] overflow-hidden hover:border-indigo-500/30 transition-all duration-500 shadow-2xl"
        >
            ...
            <div className="p-8 relative z-10">
                <div className="flex justify-between items-start mb-4">
                    <h5 className="text-xl font-black text-white leading-tight group-hover:text-indigo-400 transition-colors">
                ...
                <div className="space-y-3 mb-8">
                    <div className="flex justify-between items-end">
                        ...
                        <span className="text-[11px] font-black text-white bg-white/5 px-2 py-0.5 rounded-lg">
                    ...
                    <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <motion.div
                            ...
                            className={`h-full rounded-full ${progress >= 90 ? 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.4)]' : 'bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.4)]'}`}
                        />
                    </div>
                </div>
                ...
                <div className="flex gap-3">
                    <button
                        ...
                        className="flex-1 py-4 bg-white/5 hover:bg-white/10 rounded-2xl border border-white/10 text-[11px] font-black text-white transition-all flex items-center justify-center gap-2 group/btn"
                    >
                        عرض التفاصيل
                    ...
                    {app.status === 'completed' && app.is_certified && (
                        <button
                            ...
                            className="flex-1 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:scale-[1.02] active:scale-95 text-white rounded-2xl text-[11px] font-black transition-all flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/20"
                        >
                            <Award size={14} />
...
const MyApplications = () => {
    ...
    const getStatusDetails = (status) => {
        const config = {
            ...
            accepted: { label: 'تم القبول', color: 'text-emerald-400', bg: 'bg-emerald-400/10', icon: <CheckCircle size={14} />, border: 'border-emerald-400/20' },
            rejected: { label: 'مرفوض', color: 'text-rose-400', bg: 'bg-rose-400/10', icon: <XCircle size={14} />, border: 'border-rose-400/20' },
            completed: { label: 'مكتمل', color: 'text-green-400', bg: 'bg-indigo-400/10', icon: <FileBadge size={14} />, border: 'border-indigo-400/20' }
        };
        return config[status] || { label: 'غير معروف', color: 'text-slate-400', bg: 'bg-white/5', icon: <AlertCircle size={14} />, border: 'border-white/10' };
    };
  ...
  {/* Grid Container */}
            ...
            <main className="flex-1  p-6 lg:p-10 relative z-10 overflow-y-auto max-h-screen custom-scrollbar">
                ...
                {/* الإضاءة الخلفية */}
                <div className="fixed top-0 left-1/4 w-[500px] h-[500px] bg-indigo-600/10 blur-[120px] rounded-full pointer-events-none" />
                <div className="fixed bottom-0 right-1/4 w-[400px] h-[400px] bg-purple-600/10 blur-[100px] rounded-full pointer-events-none" />
                ...
                <div className="relative z-10 p-6 md:p-12 max-w-7xl mx-auto">
                    <header className="mb-12">
                        ...
                        >
                            ...
                            <div>
                                <h1 className="text-4xl font-black text-white tracking-tight">سجل التقديم</h1>
                                <p className="text-slate-500 text-xs font-bold mt-1">تتبع حالة طلباتك في الفرص التطوعية</p>
                            </div>
                        </motion.div>

                        {/* الفلاتر المحدثة */}
                        <div className="flex flex-wrap items-center  justify-center w-full gap-3 p-2 bg-white/[0.02] border border-white/5 rounded-[2rem] w-fit backdrop-blur-3xl">
                            {['all', 'pending', 'accepted', 'rejected', 'completed'].map((tabId) => (
                                <button
                                    key={tabId}
                                    onClick={() => setFilter(tabId)}
                                    className={`px-8 py-3 border border-white/5 rounded-[1.5rem] text-[11px] font-black transition-all duration-500 ${filter === tabId ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20 scale-105' : 'hover:bg-white/5 text-slate-500 hover:text-slate-300'}`}
                                >
                            ...
                            {filteredApplications.length > 0 ? (
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className="mt-6 text-indigo-400 font-black text-xs hover:text-indigo-300 transition-colors underline underline-offset-8"
                                    >
