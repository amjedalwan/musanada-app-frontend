            ...
            <main className="flex-1 p-8">
                <div className="max-w-4xl mx-auto">
                    <div className="flex justify-between items-center mb-8">
                        <div>
                            <h1 className="text-3xl font-bold text-emerald-600">مهاراتي الشخصية</h1>
                            <p className="text-slate-500 mt-2">اختر المهارات التي تتقنها لتظهر في ملفك الشخصي وتزيد فرص قبولك.</p>
                        </div>
                        <button
                            onClick={handleSave}
                            disabled={saving}
                            className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 px-6 py-3 rounded-2xl font-bold transition-all disabled:opacity-50"
                        >
                    ...
                    <div className="bg-white border border-slate-100 p-6 rounded-3xl mb-8">
                        <h2 className="text-sm font-bold text-slate-400 uppercase mb-4 tracking-wider">المهارات المضافة ({mySkills.length})</h2>
                        <div className="flex flex-wrap gap-3">
                            {mySkills.length > 0 ? mySkills.map(skill => (
                                <span key={skill.id} className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-200 text-emerald-600 px-4 py-2 rounded-xl text-sm">
                                    {skill.name}
                                    <X size={14} className="cursor-pointer hover:text-slate-900" onClick={() => toggleSkill(skill)} />
                                </span>
                            )) : <p className="text-slate-400 italic">لم تقم بإضافة أي مهارة بعد...</p>}
                        </div>
                    </div>
                    ...
                    <div className="relative mb-6">
                        ...
                        <input
                            ...
                            className="w-full bg-white border border-slate-200 rounded-2xl py-4 pr-12 pl-4 outline-none focus:border-purple-500/50"
                            value={searchTerm}
